...
## 2024-05-24 - Unpaginated WP_Query Defaults
**Learning:** Even when `posts_per_page` is set to `-1` or queries only fetch IDs (`fields => ids`), `WP_Query` defaults to running `SQL_CALC_FOUND_ROWS` behind the scenes unless explicitly disabled.
**Action:** Always add `'no_found_rows' => true` to any `WP_Query` arguments array where pagination is not being used or `posts_per_page` is `-1`.
