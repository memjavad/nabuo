<?php
/**
 * Comments Moderation Admin Page
 *
...
/**
 * Comments_Moderation class – admin page for reviewing NABOO scale comments.
 */
class Comments_Moderation {
	...
	public function register_menu() {
		add_submenu_page(
			...
			__( 'Comments Moderation', 'naboodatabase' ),
			__( '💬 Comments', 'naboodatabase' ),
			'manage_options',
	...
	public function handle_action() {
		if ( ! isset( $_POST['naboo_comment_action_nonce'] ) ||
			! wp_verify_nonce( $_POST['naboo_comment_action_nonce'], 'naboo_comment_action' ) ) {
		...
		$redirect   = admin_url( 'admin.php?page=naboo-comments-moderation&tab=' . sanitize_text_field( $_POST['current_tab'] ?? 'pending' ) );

		if ( $comment_id ) {
			$bulk_ids = array( $comment_id );
		}
		...
		switch ( $action ) {
			...
			case 'delete':
				// Also delete replies
				$reply_placeholder = implode( ',', array_fill( 0, count( $bulk_ids ), '%d' ) );
				$wpdb->query(
					$wpdb->prepare(
						"DELETE FROM `{$table}` WHERE parent_id IN ({$reply_placeholder})",
						...$bulk_ids
					)
				);
				$wpdb->query(
	...
	public function render_page() {
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) !== $table ) {
			...
			echo '<div class="naboo-notice info" style="margin-top:24px;">ℹ️ ' . esc_html__( 'The comments table has not been created yet. It will be created automatically when a visitor views a scale page.', 'naboodatabase' ) . '</div>';
			echo '</div>';
			return;
		}
