...
$where_args[]    = $active_tab;
		...
		if ( $search ) {
			...
			$where_clauses[] = '(r.review LIKE %s OR u.display_name LIKE %s)';
$where_args[]    = $like;
$where_args[]    = $like;
		}
		...
		$query_string = "SELECT r.*, p.post_title AS scale_title, u.display_name, u.user_email
				 FROM `{$table}` r
		...
		$ratings     = $wpdb->get_results( $wpdb->prepare( $query_string, $merged_args ) );
