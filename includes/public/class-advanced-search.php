...
class Advanced_Search {
	...
	public function advanced_search( $request ) {
		...
		/* ── Pagination & Sorting ── */
		$sort_details = $this->build_sort_details( $params['sort'], $params['keyword'], $params['rows'] );
		$order_by_sql  = $sort_details['order_by_sql'];
		$relevance_col = $sort_details['relevance_col'];
		$join_views    = $sort_details['join_views'];
		...
		if ( is_array( $rows_raw ) ) {
			foreach ( $rows_raw as $row ) {
				...
				$operator = in_array( strtoupper( $row['operator'] ?? 'AND' ), array( 'AND', 'OR', 'NOT' ), true )
					...
					: 'AND';
				if ( $term !== '' ) {
					$params['rows'][] = compact( 'term', 'field', 'operator' );
				}
			}
		}
	...
	/**
	 * Builds WHERE clauses and SQL arguments based on sanitized parameters.
	 */
	...
	/**
	 * Builds SQL for sorting.
	 */
	private function build_sort_details( $sort, $keyword, $rows ) {
		global $wpdb;
